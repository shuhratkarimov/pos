...
export default function ProfileMenu() {
  ...
  const [smsBalance, setSmsBalance] = useState<number | null>(null)   // so'mda
  ...
  useEffect(() => {
    ...
    const fetchSmsBalance = async () => {
      try {
        ...
        setSmsBalance(bal)
        setSmsRemaining(Math.floor(bal / 200))   // 200 so'm = 1 SMS
    ...
    // ixtiyoriy: har 2-3 daqiqada yangilab turish mumkin
    // const interval = setInterval(fetchSmsBalance, 180000)
          ...
          <div className="py-1">
            ...
            {user.role !== 'admin' && (
              <MenuItem
                ...
                onClick={() => setActiveModal('shopName')}
              />
            )}
            ...
            <div className="relative">
              ...
              {/* Nested options */}
              {supportOpen && (
                <div className="ml-6 mt-1 flex flex-col bg-gray-50 rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                  <button
            ...
            {/* SMS Balans ko'rsatish — eng chiroyli joy shu */}
                ...
                {balanceLoading ? (
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-600">
                      {smsRemaining} ta SMS qoldi
                      {smsBalance < 1000 && (
                        <span className="ml-1 text-red-500 font-medium">⚠️ Kam</span>
                      )}
                    </p>
                  </div>
                )}
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        {!hideSaveButton && (
          <div className="flex gap-3 p-6 pt-0">
            <button
              onClick={onSave}
              className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-xl transition-colors duration-200"
            >
              <Save size={18} />
              Saqlash
            </button>
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <button
        type="button"
        onClick={onChange}
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${checked ? 'bg-blue-600' : 'bg-gray-200'
          }`}
      >
        <span
